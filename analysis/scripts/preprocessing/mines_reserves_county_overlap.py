...
mines_zip = os.path.join(base_dir, 'facility_data', 'mines', 'epa_uldb.zip')
mines_gdf = gpd.read_file(f'zip://{mines_zip}')
...
mines_result_df = mines_result_df.merge(mines_gdf[['ICF_ID', 'DOCS']], on='ICF_ID', how='left')
...
reserves_zip = os.path.join(base_dir, 'facility_data', 'mines', 'EIA_NURE.zip')
reserves_gdf = gpd.read_file(f'zip://{reserves_zip}')
...
# --- Export to Excel ---
output_path = os.path.join(base_dir, 'demographic_data', 'county_overlaps', 'mines_reserves_county_overlap.xlsx')
...
print("✓ Created mines_reserves_county_overlap.xlsx")