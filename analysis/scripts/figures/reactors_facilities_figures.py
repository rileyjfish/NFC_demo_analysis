"""
Generate demographic proportion figures for Reactor facilities over time.
...
print("="*80)
print("GENERATING REACTOR DEMOGRAPHIC FIGURES (TIME SERIES)")
print("="*80)
...
def rebin_age_columns(df, rebin_map):
    ...
    rebinned_data = {}
    meta_cols = ['Name', 'FID', 'Type', 'Start', 'Stop', 'Reactor', 'Capacity (MWe)']
...
def deduplicate_reactor_units(df):
    """Deduplicate reactor units (e.g., COOK-1 and COOK-2) by base name.
    Takes demographic data from first occurrence.
    For metadata: uses earliest Start date and latest Stop date."""
    if 'Name' not in df.columns:
        return df
    
    # Extract base name (everything before -digit)
    df['_base_name'] = df['Name'].str.replace(r'-\d+$', '', regex=True)
    
    # For each base name, aggregate:
    # - First occurrence for demographic data
    # - Earliest Start date
    # - Latest Stop date
    # - Other metadata from first occurrence
    ...
    for col in df.columns:
        if col == '_base_name':
            ...
            agg_dict[col] = 'min'  # Earliest start
        elif col == 'Stop':
            agg_dict[col] = 'max'  # Latest stop
        else:
            agg_dict[col] = 'first'  # First occurrence
    
    df_dedup = df.groupby('_base_name', as_index=False).agg(agg_dict)
    
    # Remove helper column
    df_dedup = df_dedup.drop(columns=['_base_name'])
...
def calculate_facility_proportions_standard(data_by_year, demographic_name):
    ...
    for year, df in data_by_year.items():
        # Deduplicate reactor units (e.g., COOK-1, COOK-2 -> COOK)
        df = deduplicate_reactor_units(df)
        ...
        meta_cols = ['Name', 'FID', 'Type', 'Start', 'Stop', 'Capacity (MWe)']
...
def plot_total_population():
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f'{FACILITY}_total_population_timeseries.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()
    ...
    pop_df.to_csv(os.path.join(csv_dir, f'{FACILITY}_total_population_timeseries.csv'))
...
print("\n" + "="*80)
print("✓ ALL REACTOR FIGURES GENERATED!")
print("="*80)
