"""
Generate demographic proportion figures for CURIE backend facilities over time.
Creates time series visualizations with unweighted residual, weighted residual (by MTHM), 
...
mthm_weights_file = os.path.join(base_dir, 'facility_data', 'backend', 'CURIE', 'Cumulative Interim Storage.xlsx')
...
print("="*80)
print("GENERATING CURIE DEMOGRAPHIC FIGURES (TIME SERIES)")
print("="*80)
...
def extract_year_from_date(date_str):
    """Extract year from date string or datetime object"""
    if pd.isna(date_str):
        return None
    try:
...
def load_mthm_weights():
    """Load MTHM weights for CURIE facilities"""
    try:
        weights_df = pd.read_excel(mthm_weights_file, sheet_name='by_site', index_col=0)
        return weights_df
    except Exception as e:
        print(f"  ✗ Could not load MTHM weights: {e}")
        return None

def rebin_age_columns(df, rebin_map):
    """Rebin age columns according to mapping"""
    rebinned_data = {}
    meta_cols = ['FID', 'facility_name', 'operating_date', 'license_expiration_date']
...
def calculate_unweighted_residual_proportions(data_by_year, demographic_name):
    """Calculate unweighted residual proportions (facilities after operating_date)"""
    config = DEMOGRAPHIC_CONFIG.get(demographic_name, {})
    proportions_by_year = {}
    
    for year, df in data_by_year.items():
        # Filter facilities: include if year is after operating_date
        df['_operating_year'] = df['operating_date'].apply(extract_year_from_date)
        df_filtered = df[
            (df['_operating_year'].notna()) & 
            (df['_operating_year'] <= year)
        ].copy()
        df_filtered = df_filtered.drop(columns=['_operating_year'])
...
def calculate_weighted_residual_proportions(data_by_year, demographic_name, mthm_weights):
    """Calculate weighted residual proportions using MTHM weights"""
    config = DEMOGRAPHIC_CONFIG.get(demographic_name, {})
    proportions_by_year = {}
    
    if mthm_weights is None:
        return pd.DataFrame()
    ...
    for year, df in data_by_year.items():
        ...
        # Calculate weighted sums for each demographic column
        weighted_totals = {}
        for demo_col in demo_cols:
            weighted_sum = 0
            for idx, row in df_filtered.iterrows():
                fac_name = row['facility_name'] if 'facility_name' in row else idx
                # Get MTHM weight for this facility and year
                if fac_name in mthm_weights.index and year in mthm_weights.columns:
                    weight = mthm_weights.loc[fac_name, year]
                else:
                    weight = 0
                weighted_sum += row[demo_col] * weight
            weighted_totals[demo_col] = weighted_sum
        
        totals = pd.Series(weighted_totals)
...
def plot_demographic_comparison(demographic_name):
    """Create time series comparison of unweighted residual, weighted residual, and national proportions"""
    ...
    mthm_weights = load_mthm_weights()
    ...
    # Calculate proportions: Unweighted Residual, Weighted Residual, and National
    unweighted_props = calculate_unweighted_residual_proportions(facility_data, demographic_name)
    weighted_props = calculate_weighted_residual_proportions(facility_data, demographic_name, mthm_weights)
    ...
    demo_cols = [col for col in unweighted_props.columns 
                 if col in national_props.columns and col in weighted_props.columns 
    ...
    if demographic_name == 'race_ethnicity':
        unweighted_props = unweighted_props[unweighted_props.index != 2013]
        weighted_props = weighted_props[weighted_props.index != 2013]
        national_props = national_props[national_props.index != 2013]
    
    # Create single plot with all categories
    fig, ax = plt.subplots(figsize=(16, 8))
    
    all_years = sorted(set(list(unweighted_props.index) + list(weighted_props.index)))
    ...
    for i, col in enumerate(demo_cols):
        # Plot Unweighted Residual proportions
        unweighted_years = [y for y in all_years if y in unweighted_props.index]
        if unweighted_years:
            ax.plot(unweighted_years, unweighted_props.loc[unweighted_years, col], 
                    marker='o', label=f'{get_label(col)} (Unweighted Residual)', linewidth=2, markersize=6,
                    color=colors[i], linestyle='-')
        
        # Plot Weighted Residual proportions
        weighted_years = [y for y in all_years if y in weighted_props.index]
        if weighted_years:
            ax.plot(weighted_years, weighted_props.loc[weighted_years, col],
                   marker='^', label=f'{get_label(col)} (Weighted Residual)', linewidth=2, markersize=6,
    ...
    unweighted_props.to_csv(os.path.join(csv_dir, f'{FACILITY}_{demographic_name}_unweighted_proportions.csv'))
    weighted_props.to_csv(os.path.join(csv_dir, f'{FACILITY}_{demographic_name}_weighted_proportions.csv'))
...
def plot_total_population():
    """Plot total population (M + F) over time for unweighted and weighted residual"""
    ...
    # Calculate unweighted total population
    unweighted_totals = {}
    for year, df in sex_data.items():
        # Filter by operating_date
        df['year_extracted'] = df['operating_date'].apply(extract_year_from_date)
        df_filtered = df[df['year_extracted'] <= year].copy()
        ...
        if 'M' in df_filtered.columns and 'F' in df_filtered.columns:
            ...
            unweighted_totals[year] = total
    
    # Calculate weighted total population
    weighted_totals = {}
    for year, df in sex_data.items():
        df['year_extracted'] = df['operating_date'].apply(extract_year_from_date)
        df_filtered = df[df['year_extracted'] <= year].copy()
        ...
        # Get MTHM weights for this year (if year column exists)
        if int(year) in mthm_weights.columns:
            # Create a mapping of facility_name to MTHM weight for this year
            year_weights = mthm_weights[int(year)]
            df_filtered['mthm'] = df_filtered['facility_name'].map(year_weights).fillna(0)
        else:
            df_filtered['mthm'] = 0
        
        # Weight by MTHM
        if 'M' in df_filtered.columns and 'F' in df_filtered.columns:
            weighted_total = (df_filtered['M'] * df_filtered['mthm']).sum() + \
                           (df_filtered['F'] * df_filtered['mthm']).sum()
            weighted_totals[year] = weighted_total
    
    # Convert to Series for plotting
    unweighted_series = pd.Series(unweighted_totals).sort_index()
    
    # Create plot
    fig, ax = plt.subplots(figsize=(14, 8))
    
    years_unweighted = sorted(unweighted_series.index)
    
    ax.plot(years_unweighted, unweighted_series.loc[years_unweighted],
           marker='o', label='Total Population', linewidth=2, markersize=6,
    ...
    pop_df = pd.DataFrame({
        'total_population': unweighted_series
    })
...
print("\n" + "="*80)
print("✓ ALL CURIE FIGURES GENERATED!")
print("="*80)
