"""
Analyze demographic proportions near reactors across different regions.
Generates figures comparing exposed populations vs. regional baseline populations.
Processes both standard inclusion (operating reactors) and residual inclusion (ever-operated reactors).
...
OUTPUT_BASE = os.path.join(BASE_DIR, 'outputs', 'sensitivity_analysis', 'regional', 'in_region', 'reactors')
...
def calculate_baseline_proportions_by_region(baseline_file, demographic_config, sheet_name, expected_columns):
    ...
    Args:
        ...
        sheet_name: Sheet name to load (matching the reactor data sheets)
    ...
    try:
        ...
        if baseline_file.endswith('.nc'):
            ...
            # Load Excel file using the same sheet name as reactor data
...
def create_figure(column_name, data, demo_name, inclusion_type='standard'):
    ...
    # Save figure with new naming convention: reactors_{subtype}_{demographic}_region
...
print("\n" + "="*80)
print("✓ All reactor regional sensitivity figures generated successfully!")
print("="*80)
